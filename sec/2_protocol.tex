...
this systematically, this section defines a research protocol defined during the planning phase,
which is then used to find related studies. The research protocol is essentially a framework for
conducting the review. This section outlines the key decisions that were made with to create the
protocol by identifying the research questions and search strategy.
\begin{figure*}[th]
	\centering
...
identified other 8 studies (Step 1.8). Hence, a total of 12 studies (4 + 8) remained. A subsequent
5 frameworks were found with no accompanying papers. All 17 libraries are listed in Table
\ref{tab:dnn_papers}.
...
\begin{table}[h!]
	...
	\begin{tabular}{llllr}
		\hline
		ID    & Tool                                        & Stage & Results & Filtered \\
		\hline
		1     & Scopus                                      & S.1.4 & 182     & 4        \\
		2     & Semantic Scholar                            & S.1.8 & --      & 10       \\
		3     & DeepSeek AI \cite{noauthor_deepseek_nodate} & S.1.8 & --      & 3        \\
		\hline
		Total &                                             &       &         & 17       \\
	\end{tabular}
\end{table}
...
To evaluate the quality of these studies (in Step S.1.9), I adapted the quality appraisal
instrument suggested by \cite{zhou_map_2016}, considering 2 main aspects: report and relevance.
Regarding report, I checked whether the studies clearly tackled the problem, research questions and
inclusion/exclusion criteria defined in Steps S.1.1, S.1.2 and S.1.3 respectively. Concerning
relevance, I verified whether the studies presented relevant information to ensure its value for
practitioners. Finally, all 17 studies identified in the previous steps passed the quality checks.
...
include in the review by referencing useful tutorials and key documentation articles.

\paragraph{S.2.2 -- Research Questions.}
Here, the research questions defined in Section \ref{sec:research_questions} were refined using the
PICOC criterion suggested by \cite{keele_systematic_2007}. Apart from the following question,
RQ\textsubscript{2} and RQ\textsubscript{3} are also relevant:
...
As was already mentioned a series of Github repositories, documentation pages and tutorials are
reported. The search strategy involved using advanced \href{https://github.com/search/advanced}{Github Search}
and AI engines \cite{noauthor_deepseek_nodate, noauthor_perplexity_nodate} to identify relevant repositories.

\paragraph{Inclusion/Exclusion criteria.}
\TODO{Continue from here}
The following criteria were defined to select the material to include and exclude from the review:

\begin{itemize}
	\item \textbf{IC\textsubscript{1}}: The material is a repository.
	\item \textbf{IC\textsubscript{2}}: The material is a documentation page.
	\item \textbf{IC\textsubscript{3}}: The material is a tutorial.
	\item \textbf{IC\textsubscript{4}}: The material introduces a library or a framework. \\
	\item \textbf{EC\textsubscript{1}}: The material is not part of the categories: .
	\item \textbf{EC\textsubscript{2}}: The material is not a primary study.
	\item \textbf{EC\textsubscript{3}}: Study is not written in English.
\end{itemize}
