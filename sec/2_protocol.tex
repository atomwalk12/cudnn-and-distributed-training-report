...
\cellrefs{D101,D105,D106}, which otherwise would be infeasible to train due to the large required
training time.

\textbf{Raw performance.}
...
fair assessment, many libraries are evaluated in real-world scenarios \cellrefs{D301}.
...
performance \cellrefs{G1013,G1051}. This is most often done through focused optimizations % G1031 [RazvanTODO]
...
architectures \cellrefs{G1011,G1012}. % G1071 [RazvanTODO]
...
\cellrefs{G3011,G3013}. As an example, many frameworks assess raw performance by varying mini-batch sizes
...
criterion \cellrefs{G3012,G3013,G3061}. Without deterministic results, fair performance evaluation
is difficult.
...
operations. The former is an intrinsic algorithmic limitation \cellrefs{G4061}, while the
later must be manually managed due to the absence of a standardized API \cellrefs{G4051}.

Algorithmic and memory constraints are also present, with some algorithms being limited by high
memory usage \cellrefs{G4013}, and others requiring specialized implementations for various edge
...
and new neural network innovations, requiring continuous adaptation as new ideas emerge~\cellrefs{G4011}.
...
\textbf{LF3. Communication Overhead.}
...
\paragraph{LF2. Algorithmic limitations.}
...
On the other hand, GPU programming is a more specialized field, where few
